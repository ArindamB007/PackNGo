...
////////callbacks
	...
	if(opts.navigation==true){
		fakeHover.append(
			'<div class="scroll_arrow_left hidden-xs hidden-sm"> <i class="fa fa-angle-left"></i> </div>'
		).append(
	...
	var pieID = 'pie_'+wrap.index(),
		...
		prevNav = $('.scroll_arrow_left',wrap),
		nextNav = $('.scroll_arrow_right',wrap),
	...
	function thumbnailVisible() {
		...
		$('li', thumbs).each(function(){
			var pos = $(this).position(),
				...
				ulLeft = offDiv-offUl;
				if(ulLeft>0){
					$('.scroll_arrow_leftThumbs',camera_thumbs_wrap).removeClass('hideNav');
				} else {
					$('.scroll_arrow_leftThumbs',camera_thumbs_wrap).addClass('hideNav');
				}
				if((ulW-ulLeft)>wTh){
					$('.scroll_arrow_rightThumbs',camera_thumbs_wrap).removeClass('hideNav');
				} else {
					$('.scroll_arrow_rightThumbs',camera_thumbs_wrap).addClass('hideNav');
				}
				var left = pos.left,
	...
	if(elem.length!=0){
		...
		if($(thumbs).length) {
			...
			if(!$(pagination).length) {
				$(thumbs).append('<div />');
				$(thumbs).before('<div class=".scroll_arrow_leftThumbs hideNav"><div></div></div>').before('<div class=".scroll_arrow_rightThumbs hideNav"><div></div></div>');
		...
		if(!$('.imgLoaded',slide).length){
			...
			$.each(arr, function(index, value) {
				...
				if($(thumbs).length) {
					...
					$('.camera_thumbs_cont .scroll_arrow_leftThumbs',camera_thumbs_wrap).hover(function(){
						$(this).stop(true,false).animate({opacity:1},250);
					},function(){
						$(this).stop(true,false).animate({opacity:.7},250);
					});
					$('.scroll_arrow_leftThumbs',camera_thumbs_wrap).click(function(){
						var sum = 0,
					...
					$('.camera_thumbs_cont .scroll_arrow_rightThumbs',camera_thumbs_wrap).hover(function(){
						$(this).stop(true,false).animate({opacity:1},250);
					},function(){
						$(this).stop(true,false).animate({opacity:.7},250);
					});
					$('..scroll_arrow_rightThumbs',camera_thumbs_wrap).click(function(){
						var sum = 0,
							wTh = $(thumbs).outerWidth(),
							ulW = $('ul', thumbs).outerWidth(),
							offUl = $('ul', thumbs).offset().left,
